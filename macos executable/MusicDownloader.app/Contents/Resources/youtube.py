...
def download_playlist(playlist_url, screen, directory=None):
    ...
    for music in playlist:
        ...
        screen.append_text(header + music['title'] + downloading + '\n')
...
def download_single_mp3(url, screen, directory=None, music_title=None):
    def my_hook(d):
        ...
        if d['status'] == 'finished':
            if music_title:
                screen.append_text(header + music_title + converting + '\n')
            else:
                screen.append_text(header + d['filename'] + converting + '\n')
    ...
    try:
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ydl.download([url])
            if music_title:
                screen.append_text(header + music_title + converted + '\n')
            else:
                screen.append_text(header + 'File' + converted + '\n')
    except:
